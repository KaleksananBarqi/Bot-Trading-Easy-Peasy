...
Proyek ini menggunakan `pyproject.toml` untuk manajemen dependensi modern.

**Opsi A: Instalasi via pyproject.toml (Rekomendasi)**
```bash
# Install dalam mode editable (untuk development)
pip install -e .
...
**Opsi B: Instalasi via requirements.txt (Alternatif)**
```bash
...
```bash
# Untuk running tests (pytest, dll)
pip install -e ".[dev]"
```
...
📂 Bot-Trading-Easy-Peasy/
 ...
 ├── 📦 pyproject.toml           # Konfigurasi proyek & dependensi (Modern)
 └── 📦 requirements.txt         # Daftar dependensi Python (Legacy)
```
